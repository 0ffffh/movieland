...
insert into movie (id, name_ru, name_native,  release_date, description, rating, price, picture_path, votes)
...
insert into movies_countries (movie_id, country_id) values(20, 5);
...
values(1, 'Anonymous', 'anonymous.anonymous@example.com', crypt('paco',  gen_salt('bf')));
insert into users(id, nickname, email, password)
values(2, 'Рональд Рейнольдс', 'ronald.reynolds66@example.com', crypt('paco',  gen_salt('bf')));
insert into users(id, nickname, email, password)
values(3, 'Дарлин Эдвардс', 'darlene.edwards15@example.com', crypt('bricks', gen_salt('bf')));
insert into users(id, nickname, email, password)
values(4, 'Габриэль Джексон', 'gabriel.jackson91@example.com', crypt('hjkl',  gen_salt('bf')));
insert into users(id, nickname, email, password)
values(5, 'Дэрил Брайант', 'daryl.bryant94@example.com', crypt('exodus',  gen_salt('bf')));
insert into users(id, nickname, email, password)
values(6, 'Нил Паркер', 'neil.parker43@example.com', crypt('878787',  gen_salt('bf')));
insert into users(id, nickname, email, password)
values(7, 'Трэвис Райт', 'travis.wright36@example.com', crypt('smart',  gen_salt('bf')));
insert into users(id, nickname, email, password)
values(8, 'Амелия Кэннеди', 'amelia.kennedy58@example.com', crypt('beaker',  gen_salt('bf')));
insert into users(id, nickname, email, password)
values(9, 'Айда Дэвис', 'ida.davis80@example.com', crypt('pepsi1',  gen_salt('bf')));
insert into users(id, nickname, email, password)
values(10, 'Джесси Паттерсон', 'jessie.patterson68@example.com', crypt('tommy',  gen_salt('bf')));
insert into users(id, nickname, email, password)
values(11, 'Деннис Крейг', 'dennis.craig82@example.com', crypt('coldbeer',  gen_salt('bf')));


insert into users(id, nickname, email, password)
values(12, 'ADMIN', 'admin@admin.com', '$2a$12$CyyiDMbFDqyYhazAs.CFN.1C/SItMrOQP0Kz30msC1Dr.evQbTlcq');
insert into users(id, nickname, email, password)
values(13, 'USER', 'user@user.com', '$2a$12$60T2XM9DKnmkKg7pqYqIkujhJFFHLqtDJCDm7c0MvQ2Cs88eDWJFG');
...
SELECT setval('country_id_sequence', COALESCE((SELECT MAX(id)+1 FROM country), 1), true);
SELECT setval('genre_id_sequence', COALESCE((SELECT MAX(id)+1 FROM genre), 1), true);
SELECT setval('movie_id_sequence', COALESCE((SELECT MAX(id)+1 FROM movie), 1), true);
SELECT setval('review_id_sequence', COALESCE((SELECT MAX(id)+1 FROM review), 1), true);
SELECT setval('user_id_sequence', COALESCE((SELECT MAX(id)+1 FROM users), 1), true);
