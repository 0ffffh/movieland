...
public class NbuCurrencyService implements ExternalCurrencyService {
    ...
    public List<Currency> getCurrencyRate() {
        return webClient.baseUrl(NBU_URL).build().get().retrieve().bodyToMono(new ParameterizedTypeReference<List<Currency>>() {
        }).doOnError(CurrencyServiceException::new).block();
    }
